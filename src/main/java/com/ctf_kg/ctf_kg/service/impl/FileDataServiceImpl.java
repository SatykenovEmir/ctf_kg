...
import com.ctf_kg.ctf_kg.exception.NotFoundException;
...
//        document.set(file.getSize());
    ...
    public FileResponse uploadResume(MultipartFile file, Long id) {
        User user = userRepository.findById(id).orElseThrow(() ->
                new org.webjars.NotFoundException("user not found!" + id));
        if (user.getRole() == Role.SMI) {
            if (user.getFileSMI() != null) {
                FileData fileData = new FileData();
                fileData = user.getFileSMI();
                user.setFileSMI(null);
                FileData save = uploadFile(file, fileData);
                user.setFileSMI(save);
                userRepository.save(user);
                return fileMapper.toDto(save);
            } else {
                FileData fileData = uploadFile(file);
                user.setFileSMI(fileData);
                userRepository.save(user);
